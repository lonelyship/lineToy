	...
	private SensorEventListener m_sensorEventListener = new SensorEventListener()
	{
		...
		public void onSensorChanged(SensorEvent event)
		{
			...
			if(sensorType == Sensor.TYPE_ACCELEROMETER) {
			  ...
			  *就OK了~~~
			  */
              //  Log.e("搖一搖",("X : " +Math.abs(values[0])+" Y:"+(Math.abs(values[1]))+" Z: "+(Math.abs(values[2]))));

				if((Math.abs(values[0])>18 || Math.abs(values[1])>18)) {
					if (bSensor2Workflag)
					{
						bSensor2Workflag = false;

						m_onListener.onMShakeWork(50);
					}
				}else if((Math.abs(values[0])>15 || Math.abs(values[1])>15)) {
                    if (bSensor2Workflag) {
                        bSensor2Workflag = false;

                        m_onListener.onMShakeWork(30);
                    }
                }else if((Math.abs(values[0])>11 || Math.abs(values[1])>11)) {
                    if (bSensor2Workflag)
                    {
                        bSensor2Workflag = false;

                        m_onListener.onMShakeWork(10);
                    }
                }else if((Math.abs(values[0])>8 || Math.abs(values[1])>8)) {
                    if (bSensor2Workflag)
                    {
                        bSensor2Workflag = false;

                        m_onListener.onMShakeWork(5);
                    }
                }
